...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('ostrzezenie')
        .setDescription('[Management] Wystaw ostrzezenie.')
        .addUserOption(option =>
            option
                .setName('user')
                .setDescription('Osoba której chcesz dać ostrzezenie')
                .setRequired(true))
        .addStringOption(
    ...
    async execute(interaction) {
        ...
        const permslist = [
            ...
            '1279774041670160456', // bot.ostrzezenia 
        ];
        ...
        const nadanominusa = new EmbedBuilder()
            ...
            .setTimestamp()
            .setDescription(`Pomyślnie nadano ostrzeżenie ${user}.`)
        
        const plus1 = '1279773999865532489';
        const plus2 = '1279774001169694730';
        const userRoles2 = interaction.guild.members.cache.get(user.id).roles.cache;
        let liczba = 0;
        const plus = new EmbedBuilder()
            .setColor(`DarkRed`)
            .setTitle('Ktoś Otrzymał Ostrzeżenie')
        ...
        if (!userRoles2.has(plus1) && !userRoles2.has(plus2)){
            ...
            liczba = 1;
            plus.addFields(
                { name: "Które to ostrzeżenie:", value: `${liczba}/2`, inline: true},
                { name: "Powód:", value: `${powod}`, inline: true },
            )
            interaction.client.channels.cache.get(`1279774471397314582`).send({
        ...
        if (!userRoles2.has(plus1) && userRoles2.has(plus2)){
            const minus3er = new EmbedBuilder()
                ...
                .setTitle('Błąd!')
                .setThumbnail('https://imgur.com/35ScGn8.png')
                .setDescription(`${interaction.options.getUser('user')} posiada już 2 ostrzeżenia!\nNie mogę mu nadać następnego.`)
        ...
        db.query("INSERT INTO ostrzezenia (id, imie_nazwisko, stopien, odznaka, powod, ktore_ostrzezenie, wystawiajacy, data) VALUES (?,?,?,?,?,?,?,?)",[user.id, name, oldRole2.name, odznakast, powod, `${liczba}/2`, interaction.member.displayName, new Date()], (err,results) => {
            if (err) {
