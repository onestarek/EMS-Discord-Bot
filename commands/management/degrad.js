...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('degrad')
        .setDescription('[MANAGEMENT] Wystaw degradacje.')
    ...
    async execute(interaction) {
        ...
        try{
            ...
            try{
                await member.setNickname(`[${odznaka}] ${name}`,`Degradacja`)
            } catch(error){
                console.log(error);
            ...
            .setFooter({ text: `Uruchomione przez: ${interaction.user.tag}` })
            .setTimestamp()
            await interaction.client.channels.cache.get(`1279774427860435131`).send({
            ...
            db.query("INSERT INTO degradacje (id, imie_nazwisko, stary_stopien, nowy_stopien, stara_odznaka, nowa_odznaka, powod, wystawiajacy, data) VALUES (?,?,?,?,?,?,?,?,?)",[user.id, name, oldRole2.name, nowystopien.name, odznakast, odznaka, powod, interaction.member.displayName, new Date()], (err,results) => {
                if (err) {
                    console.error('Error executing query:', err);
                    return;
                }
            })
            db.query(`UPDATE pracownicy SET data_degradacji = NOW(), stopien = ?, odznaka = ? WHERE id = ?`, [nowystopien.name, odznaka, user.id], (err,results) => {
                if (err) {
                    console.error('Error executing query:', err);
                    return;
                }
